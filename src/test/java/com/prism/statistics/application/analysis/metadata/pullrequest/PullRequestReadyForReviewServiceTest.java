...
class PullRequestReadyForReviewServiceTest {
    ...
    @Test
    void Pull_Request_ready_for_review_이벤트를_처리하면_OPEN_상태로_변경된다() {
        // given
        PullRequestReadyForReviewRequest request = createReadyForReviewRequest();

        // when
        processingSourceContext.withInboxProcessing(() -> pullRequestReadyForReviewService.readyForReview(TEST_PROJECT_ID, request));
    ...
    @Sql("/sql/webhook/insert_project_and_draft_pull_request.sql")
    @Test
    void Pull_Request_ready_for_review_이벤트를_처리하면_PullRequestStateChangedEvent가_발행된다() {
        // given
        PullRequestReadyForReviewRequest request = createReadyForReviewRequest();
    ...
    @Sql("/sql/webhook/insert_project_and_draft_pull_request.sql")
    @Test
    void Pull_Request_ready_for_review_이벤트를_처리하면_StateHistory가_저장된다() {
        // given
        PullRequestReadyForReviewRequest request = createReadyForReviewRequest();
        ...
        assertAll(
                () -> assertThat(history.getPreviousState()).isEqualTo(PullRequestState.DRAFT),
    ...
    @Test
    void DRAFT_상태가_아닌_PullRequest에_ready_for_review_이벤트가_오면_상태가_변경되지_않고_이벤트가_발행되지_않는다() {
        // given
        PullRequestReadyForReviewRequest request = createReadyForReviewRequest();
