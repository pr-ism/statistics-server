...
class PullRequestReopenedControllerTest extends CommonControllerSliceTestSupport {
    ...
    @Test
    void PullRequest_reopened_이벤트_수집_성공_테스트() throws Exception {
        // given
        String payload = """
                {
                    "runId": 12345,
        ...
        PullRequest_reopened_이벤트_수집_문서화(resultActions);
    }

    private void PullRequest_reopened_이벤트_수집_문서화(ResultActions resultActions) throws Exception {
        resultActions.andDo(
                restDocs.document(
                        requestHeaders(
                        ...
                        requestFields(
                                ...
                                fieldWithPath("reopenedAt").description("다시 열린 일시")
                        )
                )
        );
    }
