...
public class CollaborationStatisticsQueryService {
    ...
    private ReviewerConcentrationStatistics buildReviewerConcentration(Map<Long, Long> reviewerReviewCounts) {
        ...
        double top3ReviewerRate = calculateTop3ReviewerRate(counts);
    ...
    private double calculateGiniCoefficient(List<Long> values) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            giniSum += (2.0 * (i + 1) - n - 1) * sorted.get(i);
        }

        return giniSum / (n * sum);
    }

    private double calculateTop3ReviewerRate(List<Long> counts) {
        if (counts.isEmpty()) {
            return 0.0;
        }
    ...
    private List<AuthorReviewWaitTime> buildAuthorReviewWaitTimes(List<AuthorReviewWaitTimeDto> dtos) {
        ...
        return dtos.stream()
                .map(dto -> {
                    double avgReviewWaitMinutes = dto.prCount() > 0
                            ? (double) dto.totalReviewWaitMinutes() / dto.prCount()
                            : 0.0;
    ...
    private List<ReviewerStats> buildReviewerStats(
        ...
        return reviewerReviewCounts.entrySet().stream()
                .map(entry -> {
                    ...
                    String reviewerName = responseTime != null ? responseTime.reviewerName() : "Unknown";
                    double avgResponseTimeMinutes = 0.0;

                    if (responseTime != null && responseTime.reviewCount() > 0) {
                        avgResponseTimeMinutes = (double) responseTime.totalResponseTimeMinutes() / responseTime.reviewCount();
                    }
