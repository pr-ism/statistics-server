...
```mermaid
flowchart LR
    A["GitHub Repository<br/>PullRequest 이벤트 발생"] -->|"webhook / workflow"| B["GitHub Actions<br/>GraphQL로 상세 데이터 조회"]
    B -->|"POST /collect/** + X-API-Key"| C["Statistics Server<br/>메타데이터 저장 → 이벤트 발행<br/>→ 비동기 파생 지표 계산"]
    C --> D[("MySQL<br/>metadata / history / insight")]
    D --> E["Statistics API<br/>/projects/{projectId}/statistics/**"]
    E --> F["대시보드"]
```
...
```mermaid
flowchart LR
    A(("GitHub<br/>Action")) --> B["collect"]
    B --> C["listener<br/><br/>@TransactionalEventListener<br/>phase = BEFORE_COMMIT"]

    C --> D["publisher<br/><br/>@Async"]

    D --> E["@TransactionalEventListener<br/>phase = AFTER_COMMIT"]
    D --> F["Back Fill publisher<br/><br/>@Async<br/>@TransactionalEventListener<br/>phase = AFTER_COMMIT"]
